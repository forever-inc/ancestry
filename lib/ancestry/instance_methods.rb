module Ancestry
  module InstanceMethods
    ...
    def child_ancestry
      ...
      col = self.ancestry_base_class.ancestry_column.to_s
      prev_value = self.send("will_save_change_to_#{col}?") ? changes_to_save[col][0] : self.send("#{col}")
      if prev_value.blank? then id.to_s else "#{prev_value}/#{id}" end
    end
